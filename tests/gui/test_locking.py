...
def test_delete_protection(main_window, mock_db, qtbot):
    ...
    with qtbot.waitSignal(list_widget.delete_requested, timeout=1000) as blocker:
        ...
        # doc_controller.delete_documents lazily imports from gui.utils — patch there.
        # gui.document_list imports show_selectable_message_box at module level — patch there.
        with patch('gui.document_list.show_selectable_message_box') as mock_msg_list, \
             patch('gui.utils.show_selectable_message_box', return_value=QMessageBox.StandardButton.Yes) as mock_msg_main:

             list_widget.delete_selected_documents(["u1", "l1"])

             # Verify Warning in list (for locked items)
             assert mock_msg_list.called
             # Verify Deletion confirmation in controller (for unlocked items)
